	...
	//	$('#popid_nowPage').empty().html(nowPage);
		...
		$(data.list).each(function(i) {
			...
			if(data.list[i].S_SEQ == '3555' || data.list[i].S_SEQ == '4943' || data.list[i].S_SEQ == '5032785'
				...
				cafeHtml = '<a href="#" class="ui_btn small btn_cafe" title="카페 바로가기" onclick="portalSearch(\''+data.list[i].S_SEQ+'\', \''+encodeURIComponent(data.list[i].TITLE)+'\',\''+data.list[i].SITE+'\'); return false;\">&#xe055;</a>';
			}else{
				cafeHtml = '';
			}
			...
			innerHtml += '<tr>';
			innerHtml += '<td class="title">'+cafeHtml+'<a href="http://hub.buzzms.co.kr?url='+ encodeURIComponent(data.list[i].URL) +'" target="_blank" title='+ data.list[i].TITLE +' ><span>'+ data.list[i].TITLE +'</span></a></td>';
