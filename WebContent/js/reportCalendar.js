...
yesterday_date = date.getDate()-1;
toDay = calendar_format_date(today_date, today_month, today_year);
yesterday = calendar_format_date(yesterday_date, today_month, today_year);
mindate = '2021-03-24';
...
function calendar_onclick(d, m, y) {
	...
	if(calendar_format_date(d, m, y) >= toDay) {
		alert('보고서는 전 일자까지 작성 가능합니다.');
		calendar_element.value = yesterday;
		console.log(calendar_element.value);
	}
	if(calendar_format_date(d, m, y) <= mindate) {
		alert('보고서는 운영일 이후부터 작성 가능합니다.');
		calendar_element.value = yesterday;
	}
