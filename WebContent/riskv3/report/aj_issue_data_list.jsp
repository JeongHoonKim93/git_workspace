...
<table id="board_01" border="0" cellpadding="0" cellspacing="0" style="table-layout: fixed;">
	...
	%>
	<tr>
		<td><input type="checkbox" id="majorCheck<%=i %>" name="majorCheck" value="<%=icBean.getIc_code()%>" onclick=""></td>
		<td><p style="text-overflow:ellipsis;overflow:hidden;white-space:nowrap;" title="<%=icBean.getIc_name()%>"><%=ic_name%></p></td>
		<td><%=icBean.getIc_regdate("yyyy-MM-dd")%></td>
		<td><%=icBean.getCnt()%></td>
	</tr>
	<% } %>
</table>
...
%>				
	...
	<tr>
		<td><input type="checkbox" id="TMCheck<%=i %>" name="TMCheck" value="<%=idBean.getId_seq()%>" onclick="getListCnt(<%=i%>);"></td>
...
<%-- 		<td><input id="<%=idBean.getId_seq() %>" name="TMReplyCount" type="text" style="width:100%" onKeyPress="return numbersonly(event, false);"/></td> --%>
		<td><%=idBean.getTemp3()%></td>
        <td align="center" bgcolor="#FFFFFF">
        	<input type='hidden' id='listIdx<%=i %>' name='chkIdx' value="<%=idBean.getId_seq()%>" />	
        	<input type='text' id='listCnt<%=idBean.getId_seq() %>' name='chkSeq' size="1" onchange="clickIdx(<%=i%>)" onkeyup="checkInputKey(this);" value="">	
        </td>
	</tr>
<%
			}
