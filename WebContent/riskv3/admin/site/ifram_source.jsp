				...
				<tr>
					<td width="140">
						<select name="language" class="t" style="width:140px;" onchange="search_SourceSite_frm();">
					       <option value="" <%if( language.equals("")) out.println("selected");%>>전체
					       <option value="KOR" <%if( language.equals("KOR")) out.println("selected");%>>한국어
					       <option value="ENG" <%if( language.equals("ENG")) out.println("selected");%>>영어
    					 </select>
					</td>
					<td width="30"></td>
...
<!--   </form>-->
   ...
   <tr>
     ...
     <select name="gsn" multiple style="width:305px;height:190px;" class="t">
<%
	String active = "";
	String update = "";
...
%>
	 <option <%=active%> value="<%= SGinfo.get_gsn()%>"><%=update %><%= SGinfo.get_name()%>  (<%= SGinfo.getS_reg_date() %>) </option>
<%
	}
