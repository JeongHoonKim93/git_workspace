...
/*
	...
	*/
	...
	String skr_seq = pr.getString("skr_seq");
	String k_section_type = pr.getString("k_section_type");
	String sg_seqs = pr.getString("sg_seqs");
	String s_seqs = pr.getString("s_seqs");
	String skr_type = pr.getString("skr_type");
	String skr_keyword = pr.getString("skr_keyword");
	
	System.out.println("tg = "+tg+", mode = "+mode+" ");
	System.out.println("skr_seq = "+skr_seq+" ");
	System.out.println("xp = "+xp+", yp = "+yp+", zp = "+zp+", val = "+val);
	
	
	DomainKeywordMng keymng = DomainKeywordMng.getInstance();
	...
	if( tg.equals("total") ) {
		...
		if( mode.equals("ins") ) {
			...
			out.print(" var board = window.parent.document.getElementById('board');	\n");
			out.print("	board.style.display = 'none';				 	\n");
			out.print("</SCRIPT>							\n");	
		...
		} else if(mode.equals("insertSECTION_KEY_RELATION")) {
			//영역지정그룹 내 설정정보 저장
			keymng.insertSECTION_KEY_RELATION(k_section_type,s_seqs,sg_seqs, Integer.parseInt(xp), Integer.parseInt(yp),Integer.parseInt(zp),skr_type,skr_keyword,SS_M_NO);

			Script = "?xp="+xp+"&yp="+yp+"";
			
			//페이지 새로고침
			out.print("<SCRIPT Language='JavaScript'>		\n");
			out.print("parent.loadList("+xp+","+yp+","+zp+");	\n");
			out.print("</SCRIPT>							\n");
		...
		if( mode.equals("ins") ) {
		   ...
		   StringBuffer sb = new StringBuffer();
			...
			Script = "?xp="+xp+"&yp="+yp+"&zp="+zp+"";
	} else if(tg.equals("del_board")) {
		//키워드 리스트 삭제버튼 클릭 메서드
		keymng.delBoard(skr_seq);
		//페이지 새로고침
