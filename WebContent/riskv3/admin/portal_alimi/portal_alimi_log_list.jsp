...
<%@ page import="risk.admin.portal_alimi.PortalAlimiLogMgr
              	,risk.admin.portal_alimi.PortalAlimiLogSuperBean
...
<%
	...
	String pas_type = pr.getString("pas_type");
	...
	String pal_type = pr.getString("pal_type","");
	
	int rowCnt = 10;
	int iNowPage  = pr.getInt("nowpage",1);	
	
	PortalAlimiLogMgr lMgr = new PortalAlimiLogMgr();
	
	ArrayList reData = lMgr.getAlimiLogList(iNowPage , rowCnt, sDateFrom, sDateTo, pal_type, searchKey, pas_type);
...
<!--
	...
	function pageClick( paramUrl ) {
		...
		f.action='portal_alimi_log_list.jsp'+ paramUrl;
		f.submit();
    }
	...
	//검색
 	function search(){ 		
		...
		f.nowpage.value = '1';
 		f.action = 'portal_alimi_log_list.jsp';
 		f.target = '';
 		f.submit(); 	
 	}
		...
		<table style="width:820px;" border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
				...
				<table style="width:100%;" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td><img src="../../../images/admin/portal_alimi/tit_icon.gif" /><img src="../../../images/admin/portal_alimi/tit_0918.gif" /></td>
						<td align="right">
						<table border="0" cellpadding="0" cellspacing="0">
							<tr>
								<td class="navi_home">HOME</td>
								<td class="navi_arrow">관리자</td>
								<td class="navi_arrow2">포탈 알리미 로그관리</td>
							</tr>
						</table>
						</td>
					</tr>
			...
			<tr style="display: none">
				...
				<table style="width:100%;" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td style="width:76px;"><img src="../../../images/admin/portal_alimi/btn_allselect.gif" onclick="allselect();" style="cursor:pointer;"/></td>
				        ...
				        <td width="818" bgcolor="F7F7F7" style="padding:10px 0px 5px 0px"><table width="798" border="0" align="center" cellpadding="0" cellspacing="0">
				          <tr>
				            <td height="24" style="padding:0px 0px 5px 7px"><table width="790" border="0" cellspacing="0" cellpadding="0">
				              <tr>
				              	<td width="60px" class="b_text"><img src="../../../images/issue/icon_search_bullet.gif" width="9" height="9" /> <strong>구     분</strong></td>
				                <td width="150px"><select name="pas_type" class="textbox3" style="width: 105px">
								                  <option value="">선택하세요</option>
								                  <%-- <option value="1" <%if(pas_type.equals("1")){out.print("selected");}%>>이메일</option> --%>
								                  <%-- <option value="2" <%if(pas_type.equals("2")){out.print("selected");}%>>SMS</option> --%>
								                  <option value="3" <%if(pas_type.equals("3")){out.print("selected");}%>>R-rimi</option>
				                                </select></td>
				                <td width="60px" class="b_text"><img src="../../../images/issue/icon_search_bullet.gif" width="9" height="9" /> <strong>상     태</strong></td>
				                <td><select name="pal_type" class="textbox3" style="width: 105px">
								                  <option value="">선택하세요</option>
								                  <option value="1" <%if(pal_type.equals("1")){out.print("selected");}%>>발송</option>
								                  <option value="0" <%if(pal_type.equals("0")){out.print("selected");}%>>실패</option>
								                  <option value="2" <%if(pal_type.equals("2")){out.print("selected");}%>>유사</option>
				                                </select></td>
			...
			<tr>
				...
				<col width="15%"><col width="5%"><col width="5%"><col width="60%"><col width="5%"><col width="10%">
					<tr>           
						<th>발송매체</th>
						<th>수신자</th>
						<th></th>
...
<%
	...
	if(reData.size()>0){
		PortalAlimiLogSuperBean.AlimiLogList abean = null;
		String type = "";
		String status = "";
	  	for(int i=0; i < reData.size(); i++)
		{
	  		abean = 	(PortalAlimiLogSuperBean.AlimiLogList)reData.get(i);
	  		ArrayList arrReceiverList = new ArrayList();
	  		
	  		if(abean.getPas_type().equals("1")){
	  			type = "ico_email.gif";
	  		}else if(abean.getPas_type().equals("2")){
	  			type = "ico_sms.gif";
	  		}else if(abean.getPas_type().equals("3")){
	  			type = "ico_Rrimi.gif";
	  		}
	  		
	  		if(abean.getPal_type().equals("1")){
	  			status = "발송";
	  		}else if(abean.getPal_type().equals("0")){
	  			status = "실패";
	  		}else if(abean.getPal_type().equals("2")){
	  			status = "유사";
	  		}else{
	  			status = "";
	  		}
%>
					<tr>
						<td><%=abean.getPas_title()%></td>
						<td><%=abean.getPas_cnt()+"명"%></td>
						<td style=""><img src="../../../images/admin/portal_alimi/<%=type%>"></td>
						<td style="text-align: left;"><a onClick="hrefPop('<%=abean.getPm_url()%>');" href="javascript:void(0);"><%=abean.getSend_message().toString().replaceAll("<","&lt;").replaceAll(">","&gt;")%></a></td>						
						<td style=""><%=status%></td>
						<td><%= du.getDate(abean.getPm_date(), "MM-dd HH:mm")%></td>
					</tr>
<%
		}
	}else{
