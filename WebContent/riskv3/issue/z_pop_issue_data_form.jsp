...
<!--
    ...
    function getYp()
	{    	    	
		ajax.post('../search/selectbox_kyp.jsp','fSend','getSelect');
	}
...
<input name="md_pseq" id="md_pseq" type="hidden" value="<%=idBean.getMd_pseq()%>"><!-- 기사부모번호 -->
			...
			<tr>
				...
				<table id="board_write" border="0" cellspacing="0">
					...
					<tr>
						...
						<%
						/*
							String portalCode ="30";
						
							KeywordBean kbean = null;
							ArrayList arrXp = metaMgr.getK_XPData(md_seq, (idBean.getSg_seq().equals(portalCode) ? "TOP" : ""));
							*/
						%>
						...
						/*
							...
							for(int i =0; i < arrXp.size(); i++){
								...
								selectChk = "";
								if(kbean.getKGxp().equals(idBean.getK_xp())){
									selectChk = "selected"; 
									kxp = kbean.getKGxp();
								}
						...
						<% 
						/*
								ArrayList arrYp = metaMgr.getK_YPData(md_seq, kxp, (idBean.getSg_seq().equals(portalCode) ? "TOP" : ""));
								...
								for(int i =0; i < arrYp.size(); i++){
									...
									selectChk = "";
									if(kbean.getKGyp().equals(idBean.getK_yp())){
										selectChk = "selected"; 
									}
...
%>
					...
					<tr>
						...
						<td colspan="3">
						<input type="radio" name="ra_report" value="Y" onclick="comboCheck(this)" <%if(idBean.getId_reportyn().equals("Y")){out.print("checked");}%>>포함 &nbsp;&nbsp;&nbsp;
						<input type="radio" name="ra_report" value="N" onclick="comboCheck(this)" <%if(idBean.getId_reportyn().equals("N")){out.print("checked");}%>>미포함 &nbsp;&nbsp;&nbsp;
						<input type='hidden' name='focus_ra_report' value='<%=idBean.getId_reportyn()%>'>
						</td>
					</tr>
					<tr>
						<th><span class="board_write_tit">기사 정보</span></th>
						<td colspan="3">
						<input type="checkbox" name="f_news" <%if(idBean.getF_news() != null && idBean.getF_news().equals("Y")){out.print("checked");}%> >최초기사 
						</td>
					</tr>
					<tr>
						<th><span class="board_write_tit">개인미디어 정보</span></th>
						<td colspan="3">
						<input type="text" name="media_info" style="width: 400px" value="<%=idBean.getMedia_info()%>"> 
						</td>
					</tr>
					<%--
					<tr>
