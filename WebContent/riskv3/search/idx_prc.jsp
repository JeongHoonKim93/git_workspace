 ...
 */       
    ...
    if(idxMode.equals("revert")){
    	...
    	comment = "휴지통을 비웠습니다.";	
    }else if(idxMode.equals("truncate")){
	    if(!md_seqs.equals("")){
	    	if(subMode.equals("domain")) {
	    		dkmgr.Deletedatacnt(idxMode,md_seqs,st_name);
	    	} else {
	    		smgr.Deletedatacnt(idxMode,md_seqs,st_name);
	    	}
			comment = "삭제 되었습니다";

			arrDelseq = md_seqs.split(",");
  			if(!md_seqs.equals(""))
			{
				//로그 저장
				logBean = new LogBean();		
				logBean.setL_kinds(LogConstants.getInformationKindsVal());
...
<!--    	
        ...
        //alert(top.bottomFrame.leftFrame.location);
  		...
  		}else if(stname == "PORTAL_SEARCH_"){
  			parent.document.location = "search_news_contents.jsp?nowpage=<%=nowpage%>";
  		}else{
