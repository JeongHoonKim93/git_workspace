...
try{    
    ...
    */
    ...
    //급상승 키워드 타입(증가율,증가량,수집량)
    String upKeywordType = pr.getString("upKeywordType", "1");
    int upKeywordIndex = 0;
    upKeywordIndex = (Integer.parseInt(upKeywordType) - 1);
    
    //오늘유사정보TOP
    ArrayList sameListTop = new ArrayList();
	String s_xp = "";
	if(SS_M_NO.equals("1")){
		s_xp = (mgm.getMGBean(SS_MG_NO)).getMGxp();
	}else{
    	s_xp = (md.getMember("M_SEQ", SS_M_NO)).getMg_seq();
	}
    sameListTop = smgr.getSameListTop(s_xp);
 ...
 /*   	ArrayList arrIssueBean = new ArrayList();
	...
	*/
    ...
    chartData = smgr.getChartData(sCurrDate, uei.getK_xp(), uei.getK_yp(), uei.getK_zp()); 
%>
...
<!--
    ...
    */
    function doSearch(){
    	document.fSend.action = 'search_main_contents.jsp';
    ...
    */
    ...
    function selectTopKeyword(day, k_xp, k_yp, k_zp){
    	//검색 날짜 변경
    	setTargetDate(day);
    	var f = document.getElementById('fSend');
		f.xp.value = k_xp;
	...
	* 이슈등록 팝업 열기
	*/
	function send_issue(mode, md_seq, writername){
		//close_menu();
		...
		if(mode=='insert'){
			...
			document.send_mb.writername.value = writername;	
	...
	*/	
	function chkSiteGroupAll(chk){
	 ...
	 //급상승키워드관련
    function upKeywordList(type, obj){
		var f = document.getElementById('fSend');
        f.upKeywordType.value = type;
        
		var keyObj = document.getElementsByClassName('upKeywordMenu');
        if(keyObj){
            if(keyObj.length){
                for(var i = 0; i < keyObj.length; i++){
                	//keyObj[i].background = keyObj[i].background.replace('on', 'off');
                	setOff(keyObj[i]);

                }
            }
        }
        //obj.background = obj.background.replace('off', 'on');
        setOn(obj);
        
        ajax.post("upKeyword.jsp", "fSend", "upKeywordArea");
    }
	...
	function listAlter(md_seq, md_pseq){
 	...
 	function setOn(obj)
    {
 		var oSrc = obj.style.backgroundImage;
    	re = /_off./i;
    	var nSrc = obj.style.backgroundImage.replace(re, "_on.");
    	obj.style.backgroundImage = nSrc;
    }
    function setOff(obj)
    {
    	var oSrc = obj.style.backgroundImage;
    	re = /_on./i;
    	var nSrc = obj.style.backgroundImage.replace(re, "_off.");
    	obj.style.backgroundImage = nSrc;
    }
...
<!--헤드라인 레이어-->
<div id=div_show style="font-size: 12px; padding-right: 5px; display: none; padding-left: 5px; left: 140px; width:500px; height:auto; padding-bottom: 5px; padding-top: 5px; position: absolute; background-color: #f0f0f0; border: #555555 2px solid;"></div>
		...
		%> --%>
			...
			<tr>
				...
				<table style="width:100%;" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td><img src="../../images/search/tit_icon.gif" /><img src="../../images/search/tit_0102.gif" /></td>
						<td align="right">
			...
			<tr>
				...
				<form name="fSend" id="fSend" action="search_main_contents.jsp" method="post"  style="margin-bottom: 0px;" >
					...
					<input type="hidden" name="upKeywordType" id="upKeywordType" value="<%=upKeywordType%>">
...
<!-- 						<td><img src="../../images/search/btn_bookmark.gif" onclick="saveBookMark();" style="cursor:pointer"/></td>
						...
						<td>&nbsp;</td>
						<td><img src="../../images/search/btn_sensing.gif" onclick="send_issue('new');" style="cursor:pointer"/></td>
						<td>&nbsp;</td>
...
%>
					<tr>
						<th><input type="checkbox" id="chkData" name="chkAllCheck" value="" onclick="reverseAll(this.checked);"></th>
	  ...
	  //기자명 파싱
		...
		if(uei.getSearchMode().equals("ALLKEY")){	  		
			...
			/* }else if(mBean.getIssue_check().equals("T")){
				...
				managerButton += "src=\"../../images/search/btn_manage_off.gif\" title=\"이슈 미등록 정보입니다.\" onclick=\"send_issue('insert','"+mBean.getMd_seq()+"', '"+writername+"');\">";
			}
		}
		...
		if(uei.getSearchMode().equals("ALLKEY")){
        	...
        	//카페 일경우~
        	star = "";
        	if(mBean.getS_seq().equals("3555") || mBean.getS_seq().equals("4943") || mBean.getS_seq().equals("5032785")
...
%>
						...
						<td style="text-align: left;">
							<div class="board_01_tit_0<%=mBean.getMd_type()%>" style="text-overflow:ellipsis;overflow:hidden;white-space:nowrap;width:390px; text-align:left; display: inline-block; "  >
								<%=star%>
 								<%
...
<%-- 		<%
		...
		<!-- 퀵 -->
<!-- 		<td valign="top" style="padding-top: 82px;"> -->				
<%-- 				<!-- Today 유사정보 TOP -->
				<div>
					<table width="230" border="0" cellspacing="0" cellpadding="0" style="padding-bottom: 5px;">
					<tr>
						<td height="30" background="../../images/search/quick_bg_03.gif"><table width="230" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td style="padding:0px 0px 0px 5px"><img src="../../images/search/quick_title_02.gif" width="137" height="13" /></td>
							</tr>
						</table></td>
					</tr>
					<tr>
						<td><table width="230" border="0" cellpadding="0" cellspacing="1" bgcolor="D5D5D5">
<%
if(sameListTop.size() > 0){
	for(int i = 0; i < sameListTop.size(); i++){
		MetaBean mb = (MetaBean)sameListTop.get(i);
%>
							<tr>
								<td height="26" colspan="2" bgcolor="#FFFFFF"><table width="220" border="0" align="center" cellpadding="0" cellspacing="0">
									<tr>
										<td width="180" style="padding:0px 5px 0px 0p;text-overflow:ellipsis;overflow:hidden;white-space:nowrap;display:inline-block;" title="<%=mb.getMd_title()%>">
											<a onclick="hrefPop('<%=mb.getMd_url()%>')" style="cursor: pointer;"><%=mb.getMd_title() %></a></td>
											<a href="<%=mb.getMd_url()%>" target="_blank" style="cursor: pointer;"><%=mb.getMd_title() %></a></td>
										<td width="30" align="center" class="orange_s" style="color: #DD6414;"><%=mb.getMd_same_count()%></td>
									</tr>
								</table></td>
							</tr>
<%
...
%>
				...
				<!-- 급상승 키워드 -->
					<table width="230" border="0" cellspacing="0" cellpadding="0" style="padding-top: 1px;">
					<tr>
						<td height="30" background="../../images/search/quick_bg_01.gif"><table width="230" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td width="105" style="padding:0px 0px 0px 5px"><img src="../../images/search/quick_title_01.gif" width="93" height="13" /></td>
								<td width="125"><table width="115" border="0" cellspacing="0" cellpadding="0">
									<tr>
										<td width="37" height="16" align="center"  background="<%=SS_URL%>images/search/quick_tap_<%if(upKeywordType.equals("1")){out.print("on");}else{out.print("off");}%>.gif" class="white_s"  name="upKeywordMenu" onclick="upKeywordList('1', this)" style="cursor:pointer;"><table width="37" border="0" cellspacing="0" cellpadding="0">
										<td width="37" height="16" align="center" class="white_s upKeywordMenu" onclick="upKeywordList('1', this)" style="cursor:pointer; background-image: url('../../images/search/quick_tap_<%if(upKeywordType.equals("1")){out.print("on");}else{out.print("off");}%>.gif');"><table width="37" border="0" cellspacing="0" cellpadding="0">
											<tr style="color: white">
												<td align="center" class="white_s" style="padding:2px 0px 0px 0px">증가율</td>
											</tr>
										</table></td>
										<td width="2"></td>
										<td width="37" height="16" align="center" background="<%=SS_URL%>images/search/quick_tap_<%if(upKeywordType.equals("2")){out.print("on");}else{out.print("off");}%>.gif" class="white_s" name="upKeywordMenu" onclick="upKeywordList('2', this)" style="cursor:pointer"><table width="37" border="0" cellspacing="0" cellpadding="0">
										<td width="37" height="16" align="center" class="white_s upKeywordMenu" onclick="upKeywordList('2', this)" style="cursor:pointer; background-image: url('../../images/search/quick_tap_<%if(upKeywordType.equals("2")){out.print("on");}else{out.print("off");}%>.gif');"><table width="37" border="0" cellspacing="0" cellpadding="0">
											<tr style="color: white">
												<td align="center" class="white_s" style="padding:2px 0px 0px 0px">증가량</td>
											</tr>
										</table></td>
										<td width="2"></td>
										<td width="37" height="16" align="center" background="<%=SS_URL%>images/search/quick_tap_<%if(upKeywordType.equals("3")){out.print("on");}else{out.print("off");}%>.gif" class="white_s" name="upKeywordMenu" onclick="upKeywordList('3', this)" style="cursor:pointer"><table width="37" border="0" cellspacing="0" cellpadding="0">
										<td width="37" height="16" align="center" class="white_s upKeywordMenu" onclick="upKeywordList('3', this)" style="cursor:pointer; background-image: url('../../images/search/quick_tap_<%if(upKeywordType.equals("3")){out.print("on");}else{out.print("off");}%>.gif');"><table width="37" border="0" cellspacing="0" cellpadding="0">
											<tr style="color: white">
												<td align="center" class="white_s" style="padding:2px 0px 0px 0px">수집량</td>
											</tr>
										</table></td>
									</tr>
								</table></td>
					...
					<tr>
						<td height="28" background="../../images/search/quick_bg_02.gif">
						<table width="230" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td width="103" align="center" class="blue_d">키워드</td>
								<td width="36" align="center" class="blue_d"> 전일</td>
								<td width="36" align="center" class="blue_d">오늘</td>
								<td width="51" align="center" class="blue_d">변화량</td>
							</tr>
						</table>
						</td>
					</tr>
					<tr>
						<td id="upKeywordArea"></td>
					</tr>
				</table></td>
			</tr>
			<tr>
...
<script>
//급상승 키워드 
<%-- upKeywordList('<%=upKeywordType%>', document.getElementsByClassName('upKeywordMenu')[<%=upKeywordIndex%>]); --%>
