...
<%
	...
	if("product".equals(mode)){
		data_list = mgr.getDataList(type, sDate, eDate, psDate, peDate, settingCode, sg_code, orderBy);		
	}
...
%>
	...
	if(data_list.size()>0){
		...
		%>
              <tr>             	 
                 <td><%=i+1%></td>
                 <td><a href="#" class="lnk" onclick="statistics.updateAjax_con_11('<%=type%>','<%=data_list.get(i).get("IC_CODE")%>','<%=data_list.get(i).get("IC_NAME")%>');statistics.updateAjax_con_12('<%=type%>','<%=data_list.get(i).get("IC_CODE")%>','<%=data_list.get(i).get("IC_NAME")%>');return false;"><strong class="txt" title="<%=data_list.get(i).get("IC_NAME")%>"><%=data_list.get(i).get("IC_NAME")%></strong></a></td>
    ...
    <%	 }%>
			 <input type="hidden" name="param_type" id="param_type" value="<%=type%>"/>
	 		 <input type="hidden" name="param_code" id="param_code" value="<%=param_code%>"/>
	      	 <input type="hidden" name="param_name" id="param_name" value="<%=param_name%>"/>
