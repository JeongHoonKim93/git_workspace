...
public class AddressBookDao {
	...
	public ArrayList<AddressBookGroupBean> getAdressBookGroupList()
    {
    	AddressBookGroupBean abgBean = new AddressBookGroupBean();
    	ArrayList<AddressBookGroupBean> arrBean = new  ArrayList<AddressBookGroupBean>();
    	DBconn  conn  = null;
        PreparedStatement pstmt = null;
		Statement stmt = null;
		ResultSet rs = null;
	...
	public ArrayList<AddressBookGroupBean> getAdressBookGroupList(String ag_seqs)
    {
    	return getAdressBookGroupList(ag_seqs, "");
    }
	
	public ArrayList<AddressBookGroupBean> getAdressBookGroupList(String ag_seqs, String selectedAgSeq)
    {
    	AddressBookGroupBean abgBean = new AddressBookGroupBean();
    	ArrayList<AddressBookGroupBean> arrBean = new  ArrayList<AddressBookGroupBean>();
    	DBconn  conn  = null;
		...
		try{
			...
			sb = new StringBuffer();
			sb.append("SELECT AG_SEQ, AG_NAME FROM ADDRESS_BOOK_GROUP  \n");
			sb.append("	WHERE 1=1  \n");
			if(!"".equals(ag_seqs)){
				sb.append("AND AG_SEQ IN ("+ag_seqs+")\n");
			}
			if(!"".equals(selectedAgSeq)){
				sb.append("AND AG_SEQ NOT IN ("+selectedAgSeq+")\n");
			}
			sb.append("ORDER BY AG_SEQ 	\n");
	...
	public int addressCount( String psSearchWord , String psSearchGroup)
	{
		int reCount = 0;
		...
		try {
			...
			sb.append(" 	 FROM ADDRESS_BOOK A     \n");			
			sb.append(" 	 INNER JOIN ADDRESS_BOOK_GROUP B ON A.AG_SEQ = B.AG_SEQ \n");
			if( !psSearchWord.equals("") && !psSearchGroup.equals("")) {
				sb.append("WHERE  LOWER(AB_NAME) LIKE '%"+psSearchWord.toLowerCase()+"%'     \n");
				sb.append("AND  A.AG_SEQ = "+psSearchGroup+"     \n");
			} else if(!psSearchWord.equals("") && psSearchGroup.equals("")) {
				sb.append("WHERE  LOWER(AB_NAME) LIKE '%"+psSearchWord.toLowerCase()+"%'     \n");
			} else if(psSearchWord.equals("") && !psSearchGroup.equals("")) {
				sb.append("WHERE  A.AG_SEQ = "+psSearchGroup+"     \n");
			}
	...
	public ArrayList addressList( int piNowpage, int piRowCnt, String psSearchWord, String psSearchGroup )
	{		
		...
		try {
			...
			sb.append(" SELECT A.AB_SEQ AS AB_SEQ   \n");
			sb.append(" 	   ,A.AB_NAME AS AB_NAME    \n"); 
			sb.append(" 	   ,A.AB_DEPT AS AB_DEPT    \n");
			sb.append(" 	   ,A.AB_POSITION AS AB_POSITION    \n");
			sb.append(" 	   ,A.AB_MOBILE AS AB_MOBILE    \n");
			sb.append(" 	   ,A.AB_MAIL AS AB_MAIL   \n");	
			sb.append(" 	   ,A.AB_ISSUE_CHK AS AB_ISSUE_CHK   \n");
			sb.append(" 	   ,A.AB_REPORT_DAY_CHK AS AB_REPORT_DAY_CHK   \n");
			sb.append(" 	   ,A.AB_REPORT_WEEK_CHK AS AB_REPORT_WEEK_CHK   \n");			
			sb.append(" 	   ,B.AG_NAME AS AG_NAME   \n");			
			sb.append(" FROM  ADDRESS_BOOK A \n");
			sb.append(" INNER JOIN ADDRESS_BOOK_GROUP B ON A.AG_SEQ = B.AG_SEQ \n");
    ...
    public ArrayList getAddressBookByGroup(String seqList){
	 ...
	 */
	  ...
	  * 최근 수신자 추가
	  */
	  public ArrayList getlastAddress(){
			...
			try {
				...
				sb.append(" SELECT B.AB_SEQ 																											\n");
				sb.append("          ,B.AG_SEQ 																											\n");
				sb.append("          ,B.AB_NAME 																										\n");
				sb.append("          ,C.AG_NAME 																										\n");
				sb.append("   FROM ( 																													\n");
				sb.append("             SELECT C.AB_SEQ 																								\n");
				sb.append("               FROM  																										\n");
				sb.append("               ( 																											\n");
				sb.append("                   SELECT  IR_SEQ FROM ISSUE_REPORT WHERE IR_TYPE = 'S' AND IR_MAILYN ='Y' ORDER BY IR_SEQ DESC LIMIT 1 		\n");
				sb.append("               ) A, LOG B, LOG_RECEIVER C 																					\n");
				sb.append("             WHERE B.L_KINDS = 9 AND L_TYPE = 4 AND A.IR_SEQ = B.L_KEY 														\n");
				sb.append("               AND B.L_SEQ = C.L_SEQ 																						\n");
				sb.append("           )A, ADDRESS_BOOK B, ADDRESS_BOOK_GROUP C 																			\n");
				sb.append(" WHERE B.AB_SEQ = A.AB_SEQ 																									\n");
				sb.append("    AND B.AG_SEQ = C.AG_SEQ 																									\n");
				sb.append(" ORDER BY AB_NAME ASC 																										\n");
				...
				while(rs.next()){
		        	...
		        	abBean.setMab_dept(rs.getString("AG_NAME"));
		        	arrABBean.add(abBean);*/
					arrABBean.add(rs.getInt("AB_SEQ"));
				}
	    ...
	    public ArrayList<AddressBookBean> getAddressBookByGroupforDeduplication(String seqList,String ab_seq_list){
			
			ArrayList<AddressBookBean> arrABBean = new ArrayList<AddressBookBean>();
			//AddressBookBean  ABBean;
			try {
				dbconn = new DBconn();
				Statement stmt = null;
				...
				sb.append(" SELECT A.*   \n");
				sb.append(" 	   , IFNULL((SELECT GROUP_CONCAT(B.AG_NAME)    \n"); 
				sb.append(" 	   FROM ADDRESS_BOOK_GROUP B   \n");
				sb.append(" 	   , MAP_ADDRESS_BOOK C    \n");
				sb.append(" 	   WHERE B.AG_SEQ = C.AG_SEQ    \n");
				sb.append(" 	   AND C.AB_SEQ = A.AB_SEQ   \n");	
				sb.append(" 	   ),'') AS AG_NAME   \n");
				sb.append(" 	   , IFNULL((SELECT GROUP_CONCAT(C.AG_SEQ)     \n"); 
				sb.append(" 	    FROM MAP_ADDRESS_BOOK C    \n");
				sb.append(" 	   WHERE A.AB_SEQ = C.AB_SEQ     \n");
				sb.append(" 	   ),'') AS AG_SEQ    \n");
				sb.append("   FROM (   \n");
				sb.append(" 	   SELECT DISTINCT A.AB_SEQ AS AB_SEQ    \n");			
				sb.append(" 	   		, A.AB_NAME AS AB_NAME   \n");			
				sb.append(" 			, A.AB_DEPT AS AB_DEPT \n");
				sb.append(" 			, A.AB_POSITION AS AB_POSITION \n");
				sb.append(" 			, A.AB_MOBILE AS AB_MOBILE \n");
				...
				sb.append(" 			, A.AB_REPORT_DAY_CHK AS AB_REPORT_DAY_CHK   \n");
				sb.append(" 			, A.AB_REPORT_WEEK_CHK AS AB_REPORT_WEEK_CHK  \n");
				sb.append(" 			, A.AB_SMS_CHK AS AB_SMS_CHK  \n");
				sb.append(" 		  FROM  ADDRESS_BOOK A   \n");
				sb.append(" 				INNER JOIN MAP_ADDRESS_BOOK B ON A.AB_SEQ = B.AB_SEQ \n");
				sb.append("WHERE 1=1 \n");
				if(null != seqList  && !"".equals(seqList)){
				sb.append("AND B.AG_SEQ IN("+seqList+")					\n");
				}
				if(null != ab_seq_list  && !"".equals(ab_seq_list)){
				sb.append("AND A.AB_SEQ NOT IN("+ab_seq_list+")					\n");
				}
				sb.append(") A         								\n");
