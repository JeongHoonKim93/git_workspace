...
<script language="JavaScript" type="text/JavaScript">
	...
	function sel_all()
	{
		...
		if( userlist.m_seq.length ) {
			...
			{
				if( userlist.m_seq[i].checked == true ) val = false;
				else break;
			}

			for(var i=0 ; i<userlist.m_seq.length ; i++ )
			{
				userlist.m_seq[i].checked = val;
			}
		} else {
			if( userlist.m_seq.checked == true ) userlist.m_seq.checked = false;
			else userlist.m_seq.checked = true;
		}
	}
	...
	<tr>
		...
		<table style="width:820px;" border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
				...
				<table style="width:100%;" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td style="width:80px;"><img src="../../../images/admin/member/btn_allselect.gif" onclick="sel_all();" style="cursor:pointer;"/></td>
			...
			<tr>
				<td>
				<form name="userlist" action="admin_user_prc.jsp" method="post">
...
%>
					<tr>
						<td><input type="checkbox" name="m_seq" value="<%=memberinfo.getM_seq()%>"></td>
						<td onclick="location.href='admin_user_view.jsp?id=<%=memberinfo.getM_id()%>'"><%=memberinfo.getMg_name()%></td>
						<td onclick="location.href='admin_user_view.jsp?id=<%=memberinfo.getM_id()%>'"><%=memberinfo.getM_id()%></td>
						<td onclick="location.href='admin_user_view.jsp?id=<%=memberinfo.getM_id()%>'"><%=memberinfo.getM_name()%></td>
						<td onclick="location.href='admin_user_view.jsp?id=<%=memberinfo.getM_id()%>'"><%=memberinfo.getM_dept()%></td>
						<td onclick="location.href='admin_user_view.jsp?id=<%=memberinfo.getM_id()%>'"><span class="phone"><%=memberinfo.getM_hp()%></span></td>
						<td style="text-overflow:ellipsis;overflow:hidden;white-space:nowrap;" onclick="location.href='admin_user_view.jsp?id=<%=memberinfo.getM_id()%>'"><span class="mail"><%=memberinfo.getM_mail()%></span></td>
					</tr>
<%
		}
	}else{
