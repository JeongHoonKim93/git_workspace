	...
	if(receiverList!=null)
	{
		...
		for( int i=0 ; i < receiverList.size() ; i++ ) {
			...
			if(!allReceivers.equals("")){
				 allReceivers += ","+abBean.getMab_seq();
			}else {
				 allReceivers = String.valueOf(abBean.getMab_seq());
			}
		}
%>
<input type="hidden" id="receiverSeq" name="receiverSeq" value="<%=allReceivers%>">
...
%>
			<tr>
				...
				<%=abBean.getMab_name()%> <span style="font-weight:bold;color:#1886f7;"><%=abBean.getMab_dept()%></span></td>
