...
<%
	...
	String searchKeyword = pr.getString("searchKeyword", "");			//검색어
	String kor_bt = pr.getString("kor_bt", "");	//한글 초성 순서
	String eng_bt = pr.getString("eng_bt", "");	//영문 알파벳 순서
	String num_bt = pr.getString("num_bt", "");	//숫자 순서
	...
	ArrayList reArrList = new ArrayList();
	reArrList = rMgr.getRelationKeywordList(searchKeyword, kor_bt, eng_bt, num_bt);
...
function keywordSearch_kor(thisVal){
	...
	$("#num_bt").val("");
	$("#searchKeyword").val("");
	$("#searchFrm").attr("action", "relation_keyword.jsp");
	$("#searchFrm").submit();
}
...
function Search(){
	$("#searchKeyword").val( $("[name=searchWord]").val() );
	$("#searchFrm").attr("action", "relation_keyword.jsp");
	$("#searchFrm").submit();
}
...
{				
	...
	var r_length = $("#relation_select option:selected").length;
	...
	{	
		...
		if(r_length  ==  0  ){
			...
			alert("연관키워드는 2개 이상 선택되어야 합치기 가능 합니다.");
			return;			
		}else{
		...
		popup.openByPost('editFrm','relation_add.jsp',400,155,false,true,false,'trendPop');		
	}	
	else if(mode=='add')
	{
		popup.openByPost('editFrm','relation_add.jsp',400,155,false,false,false,'trendPop');
	}
	else if(mode=='update')
	{
		if(r_length == 0){
	...
	else if(mode=='del')
	{
		...
		if(r_length < 1  ){
			...
			var rkSeqs = $("#relation_select").val();
			var rkNames = "";
			var index = $("#relation_select option").index($("#relation_select option:selected"));
			
			$("#relation_select option").each(function(){
				console.log();
				 if( $(this).index("option:selected") > -1 ){
					if(rkNames == ""){
						rkNames = $(this).text();
					}else{
						rkNames += ","+$(this).text();
			...
			$("[name=rk_seqs]").val(rk_seqs);
			$("[name=rk_names]").val(rkNames);
		}
		
		popup.openByPost('editFrm','relation_edit.jsp',400,155,false,false,false,'trendPop');
		...
		<table style="width:820px;" border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
				...
				<table style="width:100%;" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td><img src="../../../images/admin/relationKey/tit_icon.gif" /><img src="../../../images/admin/relationKey/title.gif" /></td>
						<td align="right">
						<table border="0" cellpadding="0" cellspacing="0">
							<tr>
								<td class="navi_home">HOME</td>
								<td class="navi_arrow">관리자</td>
								<td class="navi_arrow2">연관키워드관리</td>
							</tr>
						</table>
						</td>
					</tr>
			...
			<tr>
				...
				<table id="search_box" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<th style="width: 58px;">검색 단어</th>
						<td style="vertical-align:middle"><input style="width:500px;vertical-align:middle" class="textbox" type="text" name="searchWord" onkeypress="Javascript:if (event.keyCode == 13) { Search(); }" style="vertical-align:middle"><img src="../../../images/admin/relationKey/btn_search.gif" style="vertical-align:middle;cursor:pointer" onclick="Search();"/></td>
					</tr>
					<tr>
						<th>한글</th>
			...
			<tr>
				...
				<table border="0" cellpadding="0" cellspacing="0">
					<tr>
						<!-- <td><img src="../../../images/admin/relation/btn_allselect.gif" style="cursor:pointer" onclick="selectAll(!document.managerForm.checkAll.checked);"/></td> -->
						<td><img src="../../../images/admin/relationKey/btn_merge.gif" style="cursor:pointer" onclick="popupEdit('','sum');"/></td>
						<td><img src="../../../images/admin/relationKey/btn_add.gif" style="cursor:pointer" onclick="popupEdit('','add');"/></td>
						<td><img src="../../../images/admin/relationKey/btn_modify.gif" style="cursor:pointer" onclick="popupEdit('','update');"/></td>
						<td><img src="../../../images/admin/relationKey/btn_del.gif" style="cursor:pointer" onclick="popupEdit('','del');"/></td>
					</tr>
				</table>				
				</td>
			</tr>
			...
			<tr>
				...
				<form name="editFrm" id="editFrm" method="post" onsubmit="return false;">
					...
					<input type="hidden" name="rk_seqs" >
					<input type="hidden" name="rk_names" >

				<select multiple name="relation_select" id="relation_select" style="display:block;width:100%;height:400px;padding:5px;border:1px solid #CFCFCF;border-top:2px solid #CB1300;outline:none;cursor: pointer;">
				<%
					if(reArrList.size() > 0){
						HashMap map = new HashMap(); 
						for(int i =0 ;i<reArrList.size(); i ++){
							map = (HashMap)reArrList.get(i);
				%>
