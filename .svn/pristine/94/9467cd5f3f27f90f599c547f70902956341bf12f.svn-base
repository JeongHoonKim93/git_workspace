...
<%
	...
	ArrayList MapxpgroupList= new ArrayList();
	MapxpgroupList = kmgr.getMapXpgroupList();
   	String GroupKind = pr.getString("sGroupKind","1");
 	String MapxpList = pr.getString("MapxpList");
...
<script language="javascript">
	...
	function loadList()
	{		
		ajax.post('map_behind_list.jsp?subCodeName=issueCode','managerForm','keyword_map_behindList');
	}
	...
	{				
		...
		if(mode=='update')
		{
			popup.openByPost('managerForm','keyword_map_edit.jsp',680,700,false,false,false,'trendPop');
		}		
 		else{
			popup.openByPost('managerForm','keyword_map_add.jsp',680,700,false,false,false,'trendPop');
		}
...
<input type="hidden" name="kSeq" id="kSeq">
		...
		<table style="width:820px;" border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
				...
				<table style="width:100%;" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td><img src="../../../images/admin/aekeyword/tit_icon.gif" /><img src="../../../images/admin/keyword_map/tit_0505.gif" /></td>
						<td align="right">
						<table border="0" cellpadding="0" cellspacing="0">
							<tr>
								<td class="navi_home">HOME</td>
								<td class="navi_arrow">관리자</td>
								<td class="navi_arrow2">회사/제품군관리</td>
							</tr>
						</table>
						</td>
					</tr>
				...
				<table id="search_box" border="0" cellpadding="0" cellspacing="0">
					<tr>
						...
						<td>
							...
							<tr>
								<td>
								<select name="sGroupKind" style="width:100px;">

									<option value="1" <%if("1".equals(GroupKind)){out.print("selected");} %> >전체</option>
									<option value="2" <%if("2".equals(GroupKind)){out.print("selected");} %> >키워드 그룹</option>
									<option value="3" <%if("3".equals(GroupKind)){out.print("selected");} %> >회사</option>
									<option value="4" <%if("4".equals(GroupKind)){out.print("selected");} %> >제품군</option>
								</select>
								</td>
						</tr>
						</table>
						...
						<td style="vertical-align:middle"><input style= "margin-left:10px;width:500px;vertical-align:middle" class="textbox" type="text" name="searchWord" onkeypress="Javascript:if (event.keyCode == 13) { Search(); }" style="vertical-align:middle"><img src="../../../images/admin/aekeyword/btn_search.gif" style="vertical-align:middle;cursor:pointer" onclick="Search();"/></td>
					</tr>
					<tr>
						<th style="height:30px;">키워드 대그룹</th>
						<td>
							<table style="color:#2f5065;" border="0" cellpadding="0" cellspacing="0">
							<tr>
								<td>
								<select name="MapxpList" style="width:100px;">
									<option value="">전체</option>
									<%
										for(int i=0;i<MapxpgroupList.size();i++){	
											kBean = new KeywordBean();
											kBean = (KeywordBean)MapxpgroupList.get(i);
											System.out.println("11111");	
									%>
									<option value="<%=kBean.getK_seq()%>" <% if( MapxpList.equals((kBean.getK_seq()))) out.print("selected");%>><%=kBean.getK_value()%></option>
									<%
										}
...
%>
			...
			<!-- 게시판 시작 -->
			<tr>
				<td id="keyword_map_behindList"></td>
			</tr>
			<!-- 게시판 끝 -->
		</table>
		</td>
