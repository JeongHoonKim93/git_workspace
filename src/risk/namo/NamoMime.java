...
public class NamoMime
{
	...
	*/
	 ...
	 * HTML본문 또는 TEXT 메시지 내용을 반환한다.
	 */
	public String getBodyContent(String encoding) throws MimeDecodeException
	{
		MimePart part;
		byte [] decodeByte;
		String decodeText = null;
		...
		try
		{
			InputStream is = new ByteArrayInputStream(part.getBodypart().getBytes());		// String 을 바이트로 바꿔서 입력 스트림으로 바꾼다. 이때 인코딩 타입을 iso-8859-1로 셋팅한다
			try
			{
				is = MimeUtility.decode(is, part.getEncoding());										// 디코딩
			}
			...
			decodeText = new String(decodeByte,encoding);	
		}
		catch(IOException ioe)
		{
			throw new MimeDecodeException("Cannot create input stream");
